...
from src.setgan2.inception import InceptionV3
...
def get_activations(files, model, batch_size=50, dims=2048, device='cpu',
                    num_workers=1, invert_rgb=False):
    ...
    if dims is None:
        testbatch = next(iter(dataloader))
        testbatch = testbatch.to(device)

        with torch.no_grad():
            pred = model(testbatch)[0]
        dims = pred.size(1)
    ...
    for batch in tqdm(dataloader):
        ...
        if invert_rgb:
            batch = batch[:, [2,1,0]]
        with torch.no_grad():
            pred = model(batch, quantize=True)[0]
        ...
        pred = pred.cpu().numpy()#pred.squeeze(3).squeeze(2).cpu().numpy()
...
def calculate_activation_statistics(files, model, batch_size=50, dims=2048,
                                    device='cpu', num_workers=1, invert_rgb=False):
    ...
    -- sigma : The covariance matrix of the activations of the pool_3 layer of
               the inception model.
    """
    act = get_activations(files, model, batch_size, dims, device, num_workers, invert_rgb=invert_rgb)
...
def compute_statistics_of_path(path, model, batch_size, dims, device,
    ...
    if path.endswith('.npz'):
        ...
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device, num_workers, invert_rgb=invert_rgb)

    return m, s


def calculate_fid_given_paths(paths, batch_size, device, dims=None, num_workers=1, eval_backbone='CLIP', 
                              resize_fn='legacy', invert_rgb=False):
    """Calculates the FID of two paths"""
    for p in paths:
        if not os.path.exists(p):
            raise RuntimeError('Invalid path: %s' % p)

    model = LoadEvalModel(eval_backbone, resize_fn, torch.device("cuda"))
...
class LoadEvalModel(object):
    def __init__(self, eval_backbone, resize_fn, device, logger=None):
        ...
        if self.eval_backbone=="CLIP":
            if resize_fn=="legacy":
                self.resizer = make_resizer("PyTorch", 'bicubic', (self.res, self.res))
            else: raise ValueError("CLIP backbone only supports legacy resize_fn")
        else:
            self.resizer = build_resizer(mode=resize_fn, size=self.res)
    ...
    def __call__(self, x, quantize=False):
        if quantize:
            x = quantize_images(x)
        else:
            x = x.detach().cpu().numpy().astype(np.uint8)
        x = resize_images(x, self.resizer, self.trsf, self.mean, self.std)
